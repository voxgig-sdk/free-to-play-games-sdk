...
class FreeToPlayGamesMakeError
{
    public static function call(?FreeToPlayGamesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeToPlayGamesContext([], null);
        }
        $op = $ctx->op ?? new FreeToPlayGamesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeToPlayGamesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeToPlayGamesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeToPlayGamesError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreeToPlayGamesError) {
