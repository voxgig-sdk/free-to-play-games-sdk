...
// FreeToPlayGames SDK feature test
//
...
class FtCtrl extends FreeToPlayGamesControl
{
...
class FtClient
{
    ...
    private FreeToPlayGamesUtility $utility;

    public function __construct(FreeToPlayGamesUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeToPlayGamesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeToPlayGamesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeToPlayGamesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new FreeToPlayGamesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (FreeToPlayGamesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeToPlayGamesConfig::make_feature($name);
    ...
    private static function buildUrl(FreeToPlayGamesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeToPlayGamesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeToPlayGamesResponse($fetched);
        }
    ...
    private function fail(FreeToPlayGamesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeToPlayGamesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
