    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREETOPLAYGAMES_TEST_GAME_ENTID JSON to run live")
        client = setup["client"]
...
def _game_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREETOPLAYGAMES_TEST_GAME_ENTID"))
    ...
    _live = env.get("FREETOPLAYGAMES_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREETOPLAYGAMES_TEST_EXPLAIN") == "TRUE",
        "live": _live,
