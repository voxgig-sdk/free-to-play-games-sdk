...
// FreeToPlayGames SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FreeToPlayGamesTestFeature extends FreeToPlayGamesBaseFeature
{
    ...
    public function init(FreeToPlayGamesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FreeToPlayGamesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FreeToPlayGamesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FreeToPlayGamesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FreeToPlayGamesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FreeToPlayGamesParam::call($ctx, $k);
