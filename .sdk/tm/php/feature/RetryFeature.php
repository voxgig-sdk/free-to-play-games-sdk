...
// FreeToPlayGames SDK retry feature
...
class FreeToPlayGamesRetryFeature extends FreeToPlayGamesBaseFeature
{
    ...
    public function init(FreeToPlayGamesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeToPlayGamesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeToPlayGamesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeToPlayGamesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
