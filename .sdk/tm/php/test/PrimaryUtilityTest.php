...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        FreeToPlayGamesSDK $client,
        FreeToPlayGamesUtility $utility
    ): FreeToPlayGamesContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new FreeToPlayGamesContext($ctxmap, null);
    ...
    private static function fixctx(FreeToPlayGamesContext $ctx, FreeToPlayGamesSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?FreeToPlayGamesError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FreeToPlayGamesError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new FreeToPlayGamesResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new FreeToPlayGamesBaseFeature();
    ...
    {
        $hook_client = FreeToPlayGamesSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends FreeToPlayGamesBaseFeature {
            public $hook_fn = null;
            public function TestHook(FreeToPlayGamesContext $ctx): void
            {
    ...
    {
        $init_client = FreeToPlayGamesSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends FreeToPlayGamesBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new FreeToPlayGamesSDK([
            'system' => [
    ...
    {
        $blocked_client = new FreeToPlayGamesSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new FreeToPlayGamesSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new FreeToPlayGamesSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = FreeToPlayGamesSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new FreeToPlayGamesOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = FreeToPlayGamesSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof FreeToPlayGamesError) ? $result->err->msg : (string)$result->err;
