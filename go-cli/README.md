...
boru-driven command-line client **and** interactive REPL for the FreeToPlayGames
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-to-play-games-cli)
make build

# 2. See usage (words, entities, env vars)
./free-to-play-games-cli --help
...
FREE_TO_PLAY_GAMES_BASE=https://api.example.com ./free-to-play-games-cli list game

# 6. No arguments -> interactive REPL
./free-to-play-games-cli
free-to-play-games> list game
free-to-play-games> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-to-play-games-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-to-play-games-cli list game
   ```
...
Run with no arguments to open a REPL (prompt `free-to-play-games>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-to-play-games-cli`. |
...
(`./free-to-play-games-cli <expr>`) and interactively (the REPL), and expressions compose the
