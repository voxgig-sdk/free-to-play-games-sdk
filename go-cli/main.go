// Command FreeToPlayGames-cli is a CLI + REPL for the FreeToPlayGames SDK,
...
const prompt = "free-to-play-games"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREE_TO_PLAY_GAMES_APIKEY (api key), FREE_TO_PLAY_GAMES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREE_TO_PLAY_GAMES_APIKEY carries the API key and
	// FREE_TO_PLAY_GAMES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_TO_PLAY_GAMES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_TO_PLAY_GAMES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeToPlayGamesSDK(opts)
