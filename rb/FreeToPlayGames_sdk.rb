...
require_relative 'FreeToPlayGames_types'
...
class FreeToPlayGamesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeToPlayGamesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeToPlayGamesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeToPlayGamesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeToPlayGamesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeToPlayGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeToPlayGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeToPlayGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FreeToPlayGamesError => err
    ...
    if fetched.is_a?(Hash)
      status = FreeToPlayGamesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeToPlayGamesError.new(
        "graphql_error", "FreeToPlayGamesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreeToPlayGamesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
