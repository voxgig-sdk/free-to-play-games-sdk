...
module FreeToPlayGamesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeToPlayGamesResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeToPlayGamesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeToPlayGamesResponse.new(fetched)
    else
