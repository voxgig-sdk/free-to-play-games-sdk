...
//     (\`new FreeToPlayGamesSDK(...)\` / \`FreeToPlayGamesSDK.test(...)\` become
//     \`FreeToPlayGamesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FreeToPlayGamesSDK'
...
// rewrite real client construction (\`new FreeToPlayGamesSDK(...)\` and any
// \`FreeToPlayGamesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeToPlayGamesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeToPlayGamesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
