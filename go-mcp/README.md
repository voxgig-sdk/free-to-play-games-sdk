# free-to-play-games-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FreeToPlayGames SDK as
two agent tools — `free-to-play-games_list` and `free-to-play-games_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-to-play-games-mcp)
make build

# 2. Provide credentials via the environment
export FREE_TO_PLAY_GAMES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-to-play-games \
  -- /absolute/path/to/free-to-play-games-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-to-play-games-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-to-play-games_list: first page of records
{ "entity": "game" }
{ "entity": "game", "query": { } }

// free-to-play-games_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-to-play-games-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-to-play-games \
     -- "$PWD"/dist/*/free-to-play-games-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-to-play-games_list` and `free-to-play-games_load` tools now appear
   in new sessions. Ask the agent to *"list game using free-to-play-games"*
   and it calls `free-to-play-games_list` with `{"entity":"game"}`.
...
```sh
export FREE_TO_PLAY_GAMES_APIKEY=sk_live_xxx            # API key
export FREE_TO_PLAY_GAMES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-to-play-games_list` tool
...
### Call the `free-to-play-games_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-to-play-games_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-to-play-games_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_TO_PLAY_GAMES_APIKEY` | API key sent with every request. |
| `FREE_TO_PLAY_GAMES_BASE` | Optional override of the API base URL. |
...
```sh
./free-to-play-games-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-to-play-games_load","arguments":{"entity":"game","query":{"id":1}}}}'
```
